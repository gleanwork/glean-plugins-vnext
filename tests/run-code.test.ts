...
describe("handleRunCode", () => {
  ...
  it("exposes full Node fs/path APIs for unrestricted filesystem work", async () => {
    // Deliberately outside skillsBaseDir: filesystem access is not scoped to
    // the skills cache or workspace. It has the plugin process's OS access.
    const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "run-code-fs-"));
    try {
      const env = parseEnvelope(
        (await handleRunCode(makeClient(() => ({})), makeServer(false), dir, {
          reset: true,
          code: `
            const subdir = path.join(${JSON.stringify(outsideDir)}, "generated");
            const original = path.join(subdir, "draft.txt");
            const renamed = path.join(subdir, "final.txt");
            await fs.promises.mkdir(subdir, { recursive: true });
            await fs.promises.writeFile(original, "hello from run_code", "utf8");
            const text = await fs.promises.readFile(original, "utf8");
            await fs.promises.rename(original, renamed);
            return {
              text,
              basename: path.basename(renamed),
              files: await fs.promises.readdir(subdir),
              existsViaSyncApi: fs.existsSync(renamed)
            };`,
        })) as never,
      );
      expect(env).toMatchObject({
        ok: true,
        value: {
          text: "hello from run_code",
          basename: "final.txt",
          files: ["final.txt"],
          existsViaSyncApi: true,
        },
      });
      expect(
        await fs.readFile(path.join(outsideDir, "generated", "final.txt"), "utf8"),
      ).toBe("hello from run_code");
    } finally {
      await fs.rm(outsideDir, { recursive: true, force: true });
    }
  });

  it("exposes require, familiar Node globals, and unrestricted builtin modules", async () => {
    const env = parseEnvelope(
      (await handleRunCode(makeClient(() => ({})), makeServer(false), dir, {
        reset: true,
        code: `
          ...
          const compressed = zlib.gzipSync(Buffer.from("hello from require"));
          await new Promise((resolve) => setTimeout(resolve, 1));
          return {
            require: typeof require,
            process: typeof process,
            processMatchesRequire: process === processModule,
            buffer: typeof Buffer,
            childProcessExecFile: typeof childProcess.execFile,
            childOutput: childProcess.execFileSync(
              process.execPath,
              ["-e", "process.stdout.write('child-ok')"],
              { encoding: "utf8" }
            ),
            roundTrip: zlib.gunzipSync(compressed).toString("utf8"),
            cwd: process.cwd(),
            url: new URL("https://example.com/a").pathname,
            fetch: typeof fetch,
            globalMatches: global === globalThis
          };`,
      })) as never,
    );
    expect(env).toMatchObject({
      ...
      value: {
        require: "function",
        process: "object",
        processMatchesRequire: true,
        buffer: "function",
        childProcessExecFile: "function",
        childOutput: "child-ok",
        roundTrip: "hello from require",
        cwd: process.cwd(),
        url: "/a",
        fetch: "function",
        globalMatches: true,
  ...
  it("captures normal Node console output without writing to MCP stdout", async () => {
    const env = parseEnvelope(
      (await handleRunCode(makeClient(() => ({})), makeServer(false), dir, {
        reset: true,
        code: `
          console.log("hello %s", "node");
          console.error("details", { code: 7 });
          return "done";`,
      })) as never,
    );
    expect(env.ok).toBe(true);
    expect(env.value).toBe("done");
    expect(env.stdout).toContain("hello node");
    expect(env.stdout).toContain("details { code: 7 }");
  });
  ...
  it("scans fallback cache roots and dispatches a unique case-insensitive alias canonically", async () => {
    const fallback = await fs.mkdtemp(path.join(os.tmpdir(), "run-code-fallback-"));
    try {
      const toolsDir = path.join(fallback, "agent_builder", "tools");
      await fs.mkdir(toolsDir, { recursive: true });
      await fs.writeFile(
        path.join(toolsDir, "get_agent.json"),
        JSON.stringify({
          name: "get_agent",
          server_id: "glean-agents",
      ...
      const env = parseEnvelope(
        (await handleRunCode(
          makeClient((toolName, args) => ({ toolName, args })),
          makeServer(false),
          [dir, fallback],
          {
            reset: true,
            code: `const r = await PTC_GET_AGENT({ id: "agent-123" }); return r.json();`,
          },
        )) as never,
      );
      expect(env).toMatchObject({
        ok: true,
        value: {
          // The model's uppercase alias dispatched the exact backend spelling.
          toolName: "get_agent",
          args: { id: "agent-123" },
        },
      });
  ...
  it("does not guess when case-insensitive tool names are ambiguous", async () => {
    // Put the case-only variants in separate roots so this test also works on
    // case-insensitive macOS/Windows filesystems.
    const lowerRoot = await fs.mkdtemp(path.join(os.tmpdir(), "run-code-lower-"));
    const upperRoot = await fs.mkdtemp(path.join(os.tmpdir(), "run-code-upper-"));
    try {
      for (const [root, name] of [
        [lowerRoot, "get_agent"],
        [upperRoot, "GET_AGENT"],
      ] as const) {
        const toolsDir = path.join(root, "demo", "tools");
        await fs.mkdir(toolsDir, { recursive: true });
        await fs.writeFile(
          path.join(toolsDir, `${name}.json`),
          JSON.stringify({ name, server_id: "srv", requires_approval: false }),
        );
      }
      const env = parseEnvelope(
        (await handleRunCode(
          ...
          {
            reset: true,
            code: `return await PTC_Get_Agent({});`,
          },
        )) as never,
      );
      expect(env.ok).toBe(false);
      expect(env.error.message).toContain("Case-insensitive name is ambiguous");
      expect(env.error.message).toContain("PTC_get_agent");
      expect(env.error.message).toContain("PTC_GET_AGENT");
    } finally {
      await Promise.all(
        [lowerRoot, upperRoot].map((root) =>
  ...
  it("clear error when calling an unknown PTC_ tool includes scanned roots", async () => {
    const env = parseEnvelope(
      (await handleRunCode(makeClient(() => ({})), makeServer(false), dir, {
        reset: true,
    ...
    expect(env.error.message).toContain("Scanned cache roots:");
    expect(env.error.message).toContain(dir);
  });
